...
class SaleRecord(models.Model):
    ...
    sale_datetime = models.DateTimeField(auto_now_add=True)
    selling_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)  # Allow null and blank
    ...
    def save(self, *args, **kwargs):
        if self.selling_price is not None:
            self.total_amount = self.selling_price * self.quantity_sold
        else:
            self.total_amount = self.product.price * self.quantity_sold  
